...
class square:
    ...
    def turnOdom(self, radians):
        ...
        try:
            (start_trans,start_rot) = self.listener.lookupTransform('/robot1/odom', '/robot1/base_footprint', rospy.Time(0))
            start_mtrx = self.listener.fromTranslationRotation(start_trans,start_rot)
        ...
        #if (!clockwise) desired_turn_axis = (0,0,-1);
        ...
        while done==False:
          ...
          try:
              (cur_trans,cur_rot) = self.listener.lookupTransform('/robot1/odom', '/robot1/base_footprint', rospy.Time(0))
              cur_mtrx = self.listener.fromTranslationRotation(cur_trans,cur_rot)
          ...
          relative_transform = np.linalg.inv(start_mtrx) * cur_mtrx

          scale, shear, angles, trans, persp = tf.transformations.decompose_matrix(relative_transform)
          angle_turned=angles[2]
   ...
   #       rospy.loginfo(start_yaw)
    #      rospy.loginfo("cur_yaw: ")
     #     rospy.loginfo(cur_yaw)
      ...
      #    angle_turned=math.fabs(cur_yaw-start_yaw)
